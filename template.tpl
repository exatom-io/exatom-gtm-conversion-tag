___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Exatom Conversion Tag",
  "categories": ["CONVERSIONS", "ANALYTICS"],
  "brand": {
    "id": "brand_dummy",
    "displayName": "Exatom",
    "thumbnail": "data:image/png;base64,"
  },
  "description": "Our cookieless conversion tag will measure the success of your forms.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "clientCode",
    "displayName": "Client Code",
    "simpleValueType": true,
    "valueValidators": [
      {
        "type": "REGEX",
        "args": [
          "[a-zA-Z0-9]{4}"
        ],
        "errorMessage": "Your client code, which is four characters long, should only include numbers or uppercase letters."
      }
    ],
    "valueHint": "4 characters",
    "help": "Enter your 4 character client code"
  },
  {
    "type": "TEXT",
    "name": "conversionCode",
    "displayName": "Conversion Code",
    "simpleValueType": true,
    "valueValidators": [
      {
        "type": "REGEX",
        "args": [
          "[a-zA-Z0-9]{5}"
        ],
        "errorMessage": "Your client code, which is four characters long, should only include numbers or uppercase letters."
      }
    ],
    "valueHint": "5 characters",
    "help": "Enter your 5 character conversion code"
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const log = require('logToConsole');
const clientCode = data.clientCode;
const conversionCode = data.conversionCode;

// Basic validation
if (!clientCode) {
  log('Exatom: Missing clientCode, aborting tag.');
  data.gtmOnFailure();
  return;
} else if (!conversionCode) {
  log('Exatom: Missing conversionCode, aborting tag.');
  data.gtmOnFailure();
  return;
}


// Build script URL
const scriptUrl = 'https://assets.exatom.io/conversion.js?clientCode=' + clientCode.toUpperCase() + '&tagCode=' + conversionCode.toUpperCase();

// Load the script asynchronously using the sandbox API
const injectScript = require('injectScript');

injectScript(
  scriptUrl,
  function () {
    log('Exatom conversion script loaded successfully: ' + scriptUrl);
    data.gtmOnSuccess();
  },
  function (err) {
    log('Exatom conversion script failed to load: ' + scriptUrl + ' - ' + err.message);
    data.gtmOnFailure();
  }
);


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://assets.exatom.io/*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 02/12/2025, 21:22:39


